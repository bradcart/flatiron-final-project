...
export default function LoadTemplate() {
    ...
    const handleFetch = () => {
        fetch(`http://localhost:3000/templates/${id}`, {
            ...
            .then(res => res.json())
            .then(res => setTemplate(res["identifier"]))
            .then(history.push(`/templates/${id}/edit`))
            .then(setId(1))
    };
    console.log(id)
    return (
        <Switch>
            ...
            <Route path="/templates">
                <Box className='viewport film-grain'>
                    ...
                    <span id='load-btn'><StyledButton label="LOAD" fullWidth={true} size='large' onClick={() => handleFetch()} /></span>
                </Box>
            </Route>
        </Switch>
    )
